...
function ShareSheet({ p, onClose }: { p: PositionView; onClose: () => void }) {
  ...
  return (
    <ShareCardSheet
      ...
      img={`/api/share/${slug}/${p.tokenId}`}
      url={`https://vaults.cash/p/${slug}/${p.tokenId}${ref?.refCode ? `?ref=${ref.refCode}` : ""}`}
      alt={`${p.market.base.symbol} / ${p.market.quote.symbol} position card`}
      fileBase={`vaults-cash-${p.market.base.symbol}-${p.tokenId}`}
      shareText="Every trade pays a fee. I'm the one collecting it."
      note={
        <>
          A live card: pair, what traders have paid this position, and where the price sits. &ldquo;Share image&rdquo; hands a tall
          version to Instagram or TikTok stories. The link opens a public page with the same numbers
